...
class MDP:
    def __init__(self, P, R, discount=None, horizon=None):
        ...
        assert not (
            discount is None and horizon is None
        ), "Can't have both discount and horizon be none"
        ...
        assert P.shape == (self.nActions, self.nStates, self.nStates), (
            "Invalid transition function: it has dimensionality "
            + repr(P.shape)
            + ", but it should be (nActions,nStates,nStates)"
        )
        assert (
            abs(P.sum(2) - 1) < 1e-5
        ).all(), (
        ...
        assert R.shape == (self.nActions, self.nStates), (
            ...
            + repr(R.shape)
            + ", but it should be (nActions,nStates)"
        )
        self.R = R
...
def build_infinite_horizon_maze_MDP(discount_factor=0.7):
    """
    ...
    # MDP object
    mdp = MDP(P, R, discount=discount_factor)
    return mdp
...
def build_finite_horizon_maze_MDP(time_horizon=10):
    ...
    # MDP object
    mdp = MDP(P, R, horizon=time_horizon)
    return mdp